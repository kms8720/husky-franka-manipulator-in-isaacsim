# Phase D — lidar-driven Husky + Franka pick/place test.
# Same as pick_place_husky_mobile.py, but the pick target comes from
# /tmp/lidar_cube_latest.json written by lidar_cube_detector.py instead of
# cube.get_world_pose() ground truth.
#
# Procedure:
...
#   4) Press PLAY, then slowly drive Husky forward with the joystick.
...
LIDAR_JSON = "/tmp/lidar_cube_latest.json"
LIDAR_STALE_SEC = 0.75
LIDAR_Z_BIAS = 0.0
...
print(
    "[L] spawned fixed stand/cube. Start lidar_cube_detector.py, then drive Husky until reach_xy is "
    f"{REACH_MIN_XY:.2f}~{REACH_MAX_XY:.2f} m. cube={cube_pos_world.tolist()}"
)

robot = SingleArticulation(prim_path=HUSKY, name="husky_franka_robot_lidar")
...
mobile_state = {
    ...
    "last_lidar_cube_w": None,
    "warned_no_lidar": False,
}
...
def _post_init():
    ...
    print("[L] init OK. waiting for lidar cube estimate. Drive slowly; stop near the cube for a cleaner grasp.")
...
def read_lidar_cube_world():
    if not os.path.exists(LIDAR_JSON):
        return None, "missing"
    try:
        with open(LIDAR_JSON, "r", encoding="utf-8") as f:
            data = json.load(f)
    except Exception as e:
        return None, f"read_error:{e}"
    age = time.time() - float(data.get("stamp", 0.0))
    if age > LIDAR_STALE_SEC:
        return None, f"stale:{age:.2f}s"
    if data.get("frame_id") != "world":
        return None, f"bad_frame:{data.get('frame_id')}"
    try:
        pos = np.array([float(data["x"]), float(data["y"]), float(data["z"]) + LIDAR_Z_BIAS], dtype=np.float64)
    except Exception as e:
        return None, f"bad_payload:{e}"
    return pos, f"ok age={age:.2f}s points={data.get('points', '?')}"
...
def on_mobile_physics_step(dt):
    ...
    try:
        ...
        lidar_cube_w, lidar_status = read_lidar_cube_world()
        gt_cube_w = np.asarray(cube.get_world_pose()[0], dtype=np.float64)
        if lidar_cube_w is None:
            if not mobile_state["warned_no_lidar"] or mobile_state["wait_step"] % 60 == 0:
                print(f"[L] waiting for lidar estimate ({lidar_status}). Run lidar_cube_detector.py.")
                mobile_state["warned_no_lidar"] = True
            return
        mobile_state["last_lidar_cube_w"] = lidar_cube_w
        mobile_state["warned_no_lidar"] = False
        cube_w = lidar_cube_w
        ...
        if not mobile_state["started"]:
            ...
            if mobile_state["wait_step"] <= 5 or mobile_state["wait_step"] % 30 == 0:
                print(
                    f"[L] WAIT reach_xy={reach_xy:.3f} m "
                    f"lidar=({cube_w[0]:+.3f},{cube_w[1]:+.3f},{cube_w[2]:+.3f}) "
                    f"gt_err={np.linalg.norm(cube_w - gt_cube_w):.3f} m "
                    f"(target {REACH_MIN_XY:.2f}~{REACH_MAX_XY:.2f})"
                )
            if REACH_MIN_XY <= reach_xy <= REACH_MAX_XY:
                mobile_state["started"] = True
                print(f"[L] START pick/place from lidar. reach_xy={reach_xy:.3f} m")
            else:
                return
        ...
        if mobile_state["step"] <= 5 or mobile_state["step"] % 30 == 0:
            ...
            print(
                ...
                f"reach_xy={reach_xy:.3f} EE_z={ee_z:+.3f} "
                f"lidar_z={float(cube_w[2]):+.3f} gt_z={float(gt_cube_w[2]):+.3f}"
            )
...
print("[L] subscribed. Start lidar_cube_detector.py, press PLAY, then drive Husky toward the stand.")
